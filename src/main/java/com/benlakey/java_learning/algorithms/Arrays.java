...
public class Arrays {
	...
	public static void rotateArray(int[] array, int offset) {
		
		reverseArray(array, 0, array.length);
		reverseArray(array, 0, offset);
		reverseArray(array, offset, array.length - offset);

	}

	private static void reverseArray(int[] array, int startIdx, int count) {

		for(int i = 0; i < count / 2; i++) {
			int tmp = array[startIdx + i];
			array[startIdx + i] = array[startIdx + count - 1 - i];
			array[startIdx + count - 1 - i] = tmp;
		}
